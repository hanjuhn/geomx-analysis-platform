...
export async function handleML({
  ...
  try {
    ...
    await capturePlot(
      ...
      deg_gene <- rownames(logCPM)[sig]

      if (length(deg_gene) < 2) {
        plot.new(); text(0.5,0.5,"DEG 부족",cex=1.4)
        return()
      }
      ...
      ## 2) 데이터 준비
      ## =============================
      X <- t(logCPM[deg_gene,,drop=FALSE])
      y <- g
      ...
      axis(2,at=1:nrow(cm_prop),labels=rownames(cm_prop),las=2)

      for (i in 1:nrow(cm_prop)) {
        for (j in 1:ncol(cm_prop)) {
          val <- round(cm_prop[i,j]*100,1)
    ...
    setStatus("ML 분류 완료 (Train/Test = 7:3)")
  } catch (err) {
